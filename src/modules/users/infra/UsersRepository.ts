...
import UpdateUserDto from '../domain/UpdateUserDto';
...
export class UsersRepository {
  ...
  async update({ id, name, email, password }: UpdateUserDto): Promise<UserRecord> {
    const user: UserRecord = await this.queryBuilder('users').where({ id }).update({
      name,
      email,
      password,
      updated_at: new Date(),
